...
PRO COLORBAR, BOTTOM=bottom, CHARSIZE=charsize, COLOR=color, DIVISIONS=divisions, $
   ...
   TITLE=title, VERTICAL=vertical, TOP=top, RIGHT=right, MINOR=minor, $
              ...
              overflowbins=overflowbins, overflowmin=overflowmin, overflowmax=overflowmax
...
ENDIF ELSE BEGIN
   ...
   ENDIF ELSE BEGIN
      ...
      ; kdm overflow
      if keyword_set(overflowbins) then begin
          if n_elements(overflowmin) eq 0 then overflowmin=1
          if n_elements(overflowmax) eq 0 then overflowmax=255
          oheight = (position[3] - position[1])*0.7
          overflowleft = position[0]
          overflowright = position[2]
          position[0] = position[0] + (oheight + oheight*0.3)
          position[2] = position[2] - (oheight + oheight*0.3)
      endif
      ;; shrink the colorbar by the height so we can add
      ;; an equilateral triangle to each end
...
ENDELSE
   ...
   if keyword_set( overflowbins ) then begin
       ...
       if n_elements(extra) ne 0 then begin
           thtestx = stregex(tag_names(extra),"^xth.*", /extract,/fold_case)
           if array_equal(thtestx,"") ne 1 then begin
               thindx = where( tag_names(extra) eq thtestx )
               othick = extra.(thindx)
           endif else othick = 1
       endif

       x = [ overflowleft, $
             overflowleft+oheight, $
             overflowleft+oheight ]
       y = [ (position[1]+position[3])/2.0, $
             position[3], $
             position[1] ]
       polyfill, x, y, color=overflowmin, /normal
       plots, [overflowleft,overflowleft+oheight], $
              [(position[1]+position[3])/2.0,position[3]], thick=othick, /norm
       plots, [overflowleft,overflowleft+oheight], $
              [(position[1]+position[3])/2.0,position[1]], thick=othick, /norm
       plots, [rebin((overflowleft+oheight)#1,2)], $
              [position[1],position[3]], thick=othick, /norm

       x = [ overflowright, $
             overflowright-oheight, $
             overflowright-oheight ]
       y = [ (position[1]+position[3])/2.0, $
             position[3], $
             position[1] ]
       polyfill, x, y, color=overflowmax, /normal
       plots, [overflowright,overflowright-(oheight)], $
              [(position[1]+position[3])/2.0,position[3]], thick=othick, /norm
       plots, [overflowright,overflowright-(oheight)], $
              [(position[1]+position[3])/2.0,position[1]], thick=othick, /norm
       plots, [rebin((overflowright-oheight)#1,2)], $
              [position[1],position[3]], thick=othick, /norm
   endif
