...
pro kdm_slider_test
  ...
  wBase = WIDGET_BASE(/COLUMN, $
                      TITLE="CW KDM Slider Test" )
  wKDM = CW_KDM_SLIDER(wBase, UVAL='Foo', max=42000, /DRAG, $
                       VALUE=24, TITLE='FooTitle' )
  WIDGET_CONTROL, wBase, /REALIZE
  XMANAGER, 'CW_KDM_SLIDER', wBase, $
...
;  ON_ERROR, 2
  ...
  WIDGET_CONTROL, stash, GET_UVALUE=state, /NO_COPY
	...
	; Set the value here.
  IF KEYWORD_SET( SET_LABEL ) THEN BEGIN
     WIDGET_CONTROL, state.value, set_value=value
  ENDIF ELSE BEGIN
     WIDGET_CONTROL, state.slider, set_value=value
     WIDGET_CONTROL, state.value, set_value=state.labels[value]
  ENDELSE
...
FUNCTION kdm_slider_get_value, $
   id, $
   SLIDER_MIN_MAX=SLIDER_MIN_MAX
...
;  ON_ERROR, 2
  ...
  WIDGET_CONTROL, stash, GET_UVALUE=state, /NO_COPY
	...
	; Get the value here
  IF KEYWORD_SET( SLIDER_MIN_MAX ) THEN BEGIN
     value = [state.minimum, state.maximum ]
  ENDIF ELSE BEGIN
...
FUNCTION kdm_slider_event, ev
  ...
  RETURN, { ID:parent, TOP:ev.top, HANDLER:0L, VALUE:value }
END

;-----------------------------------------------------------------------------

FUNCTION cw_kdm_slider, $
   parent, $
   UVALUE = uval, UNAME = uname, $ ; keep unused for external user
   ;;; TITLE PARAMS
   TITLE=TITLE, $
   ;;; SLIDER PARAMS
   DRAG=DRAG, MAXIMUM=MAXIMUM, MINIMUM=MINIMUM, $
   SCROLL=SCROLL, SUPPRESS_VALUE=SUPPRESS_VALUE, $
   TAB_MODE=TAB_MODE, UNITS=UNITS, VALUE=VALUE, $
   ;;; VALUE READOUT PARAMS
   LABELS=LABELS
...
;  ON_ERROR, 2					;return to caller
  ...
  IF NOT (KEYWORD_SET(uname))  THEN uname = 'CW_KDM_SLIDER_UNAME'
  IF NOT (KEYWORD_SET(minimum)) THEN minimum = 0
  IF NOT (KEYWORD_SET(maximum)) THEN maximum = 100
  IF NOT (KEYWORD_SET(labels)) THEN $
     LABELS = STRTRIM(FIX(makex(minimum,maximum,1)),2)
  ...
  mainbase = WIDGET_BASE(parent, UVALUE = uval, UNAME = uname, $
		EVENT_FUNC = "kdm_slider_event", $
		FUNC_GET_VALUE = "kdm_slider_get_value", $
		Pro_SET_VALUE = "kdm_slider_set_value")

  if ( N_ELEMENTS(tab_mode) ne 0 ) then $
        WIDGET_CONTROL, mainbase, TAB_MODE = tab_mode else $
           WIDGET_CONTROL, mainbase, /TAB_MODE
...
;;  state.id = WIDGET_LABEL(mainbase, VALUE = "Compound Widget Template")
  ...
  state.base = WIDGET_BASE(mainbase,/ROW)
  state.title = WIDGET_LABEL(state.base, VALUE=title )
  state.slider = WIDGET_SLIDER( state.base, UVALUE='SLIDER_VALUE', $
                                DRAG=DRAG, MAXIMUM=MAXIMUM, MINIMUM=MINIMUM, $
                                SCROLL=SCROLL, /SUPPRESS_VALUE, VALUE=VALUE, $
                                TAB_MODE=TAB_MODE, UNITS=UNITS )
  ;;; create the value readout widget
  state.value = WIDGET_LABEL(state.base, VALUE=' ' )
  ;; get string size based on maximum value (doesn't work for [-big,42] etc.)
  nlabels = n_elements(labels)-1
  size = WIDGET_INFO( state.value, STRING_SIZE=STRTRIM( labels[nlabels], 2 ) )
  width = size[0]

  ;; resize value readout (based on max) and apply initial value
  IF KEYWORD_SET(value) then vv = value else vv = 0
  WIDGET_CONTROL, state.value, XSIZE=size[0]+5, SET_VALUE=state.labels[vv]
